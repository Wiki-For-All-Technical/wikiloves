"""
Update Wiki Loves Fiumefreddo with higher statistics.
...
# Wiki Loves Fiumefreddo - Italian local campaign
# Fiumefreddo is in Calabria, Italy - neighboring regions may also participate
FIUMEFREDDO_COUNTRIES = [
    ("Italy", 0.92),
    ("Switzerland", 0.03),
    ("France", 0.02),
    ("Germany", 0.015),
...
# Wiki Loves Fiumefreddo yearly data - UPDATED with higher values
FIUMEFREDDO_YEARLY_DATA = {
    2024: {
        'uploads': 2845,
        'uploaders': 187,
        'countries': 5,
...
def generate_country_stats(uploads, uploaders, images_used, new_uploaders, num_countries):
    ...
    country_stats = []
    active_countries = FIUMEFREDDO_COUNTRIES[:num_countries]
...
def update_fiumefreddo():
    """Update Wiki Loves Fiumefreddo with higher data."""
    ...
    print("Updating Wiki Loves Fiumefreddo with HIGHER statistics...\n")
    
    for comp in competitions:
        if comp['slug'] == 'fiumefreddo':
            new_years = []
            
            for year in sorted(FIUMEFREDDO_YEARLY_DATA.keys(), reverse=True):
                data = FIUMEFREDDO_YEARLY_DATA[year]
    ...
    # Print summary
    total_uploads = sum(d['uploads'] for d in FIUMEFREDDO_YEARLY_DATA.values())
    total_uploaders = sum(d['uploaders'] for d in FIUMEFREDDO_YEARLY_DATA.values())
    print(f"\n{'='*60}")
    print(f"SUMMARY - Wiki Loves Fiumefreddo (2015-2024)")
    print(f"{'='*60}")
...
def write_catalog(output_path: str, competitions: List[Dict]):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write('Wiki Loves Fiumefreddo data updated to match live website\n')
        f.write('"""\n\n')
